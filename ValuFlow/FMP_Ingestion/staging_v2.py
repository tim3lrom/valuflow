...
# -- Calculates derived metrics and ratios used across all ValuFlow financial models
# -- Outputs to VALUFLOW.STAGING schema for downstream consumption by valuation models

# -- CHANGES FROM PREVIOUS VERSION --
# -- Previous Version: No formal versioning prior to this update
# -- Added implied_coupon_rate to VALUATION_INPUTS (Interest Expense / Total Debt)
# -- Added comment block for documentation and traceability

# -- DATA FLOW --
# -- Source:      VALUFLOW.RAW (raw FMP ingestion tables)
# -- Destination: VALUFLOW.STAGING (cleaned and enriched tables)
# -- Run ingestion.py before staging.py on any new data pull

# -- TABLES WRITTEN TO SNOWFLAKE --
# -- VALUFLOW.STAGING.FINANCIALS_CLEAN    -> merged income stmt, balance sheet, cash flow + derived fields
...
# -- DATA SOURCES --
# -- Financial Statements: VALUFLOW.RAW.INCOME_STMT, VALUFLOW.RAW.BALANCE_SHEET, VALUFLOW.RAW.CASH_FLOW_STMT
# -- Price Data:           VALUFLOW.RAW.PRICE_DATA
# -- S&P 500 Benchmark:    FMP API -> /historical-price-eod/light?symbol=^GSPC

# -- DEPENDENCIES --
# -- Requires VALUFLOW.RAW tables to be populated by ingestion.py first
# -- Snowflake credentials loaded via .env (python-dotenv)
# -- Private key authentication used for Snowflake connection (PKCS8 format)

# -- NOTES --
# -- implied_coupon_rate = Interest Expense / Total Debt
# --   Used in Hamada's Equation model (Model v1.3.4+) to estimate debt cash flows for Dmv
# --   Replaces need to hardcode coupon rate from 10-K for companies where it is not available
...
def calculate_returns():
    ...
    df = read_from_snowflake("SELECT SYMBOL, DATE, PRICE FROM VALUFLOW.RAW.PRICE_DATA")
