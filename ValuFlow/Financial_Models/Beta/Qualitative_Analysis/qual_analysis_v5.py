...
# Note: web search adds input token cost from search results.
...
Given quantitative model outputs, produce a qualitative analysis in exactly three sections:
...
(3) Company-Specific Factors — use web search to find the most recent and material news,
    earnings results, legal developments, or macro events affecting this ticker.

Strict formatting rules:
- Each section must contain exactly 3 sentences. No more, no less.
- Write every sentence in your own words. Do not quote, lift, or paraphrase fragments
  directly from search results. Fully synthesize all information into original prose.
- No markdown formatting of any kind — no headers, no dashes, no bullet points, no bold.
- No section separators or dividers between sections.
- Label each section only with its number and title on the same line as the first sentence,
  separated by a colon. Example: (1) Risk Profile: [first sentence here].
