...
# -- Previous Model = v1.3.5
# -- Added Section 7 — semi-annual Dmv calculation (discounts coupon payments + principal at Kd_pretax)
# -- Added Section 8 — writes Dmv output to VALUFLOW.MODELS.DMV_OUTPUT with timestamp
# -- overwrite=False on Snowflake write — appends each run as a historical snapshot
# -- Dmv written to VALUFLOW.MODELS schema (model outputs) not VALUFLOW.STAGING (raw/derived data)
...
print(f"\nTimestamp: {date.today()} | Ticker: {TICKER} | ICR Method: {method_label} | ICR: {avg_icr:.4f} | Rating: {rating} | Dmv: ${Dmv:,.0f}")