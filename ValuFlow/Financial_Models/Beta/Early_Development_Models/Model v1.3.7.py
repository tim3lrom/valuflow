...
# -- Previous Model = v1.3.6
# -- Added Section 9 — pulls Emv (market cap) from VALUFLOW.RAW.SHARES_OUTSTANDING
# -- Added Section 10 — writes Emv output to VALUFLOW.MODELS.EMV_OUTPUT with timestamp
# -- Updated comment block to reflect dual Dmv and Emv purpose
# -- Added SNOWFLAKE CONNECTIONS section to comment block documenting dual auth pattern
# -- Added VALUFLOW.RAW.SHARES_OUTSTANDING to DATA SOURCES
...
# -- snowflake.connector (private key auth) used for write_pandas() uploads in Sections 8 and 10
...
periods = MATURITY_YEARS * 2                            # total number of semi-annual periods
semi_annual_coupon = (implied_coupon / 2) * total_debt  # coupon payment per period
semi_annual_kd = Kd_pretax / 2                          # discount rate per period

# Discount each semi-annual coupon payment + principal at maturity
pv_coupons = sum(
...
# ============================================================
# Load private key for Snowflake write
...
conn.cursor().execute("CREATE SCHEMA IF NOT EXISTS MODELS")
conn.cursor().execute("USE SCHEMA MODELS")

success, nchunks, nrows, _ = write_pandas(
    conn, df_dmv, "DMV_OUTPUT",
    schema="MODELS",
    auto_create_table=True,
    overwrite=False      # append — preserves historical runs with timestamp
)

print(f"\nDmv written to VALUFLOW.MODELS.DMV_OUTPUT ({nrows} row)")
...
# Market cap is Emv by definition (shares outstanding * price)
...
success, nchunks, nrows, _ = write_pandas(
    conn, df_emv_output, "EMV_OUTPUT",
    schema="MODELS",
    auto_create_table=True,
    overwrite=False      # append — preserves historical runs with timestamp
)

print(f"\nEmv written to VALUFLOW.MODELS.EMV_OUTPUT ({nrows} row)")
conn.close()

print(f"\nTimestamp: {date.today()} | Ticker: {TICKER} | ICR Method: {method_label} | ICR: {avg_icr:.4f} | Rating: {rating} | Dmv: ${Dmv:,.0f} | Emv: ${Emv:,.0f}")